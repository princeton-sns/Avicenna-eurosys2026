...
if __name__ == '__main__':
    ...
    for line in f.readlines():
        allMedians.append(float(line))
    tmp = []
    i = 0
    while i <= len(allMedians)-nTrials:
        for j in range(0, nTrials):
            tmp.append(allMedians[i+j])
        if nTrials%2 == 1:
            print(tmp[int((nTrials-1)/2)])
        else:
            print((tmp[int((nTrials-1)/2)] +  tmp[int((nTrials-1)/2 + 1)])/2.0)
        tmp = []
        i += nTrials
