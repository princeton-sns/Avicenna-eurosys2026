...
const BEACON_FAILURE_THRESHOLD = 1000
...
const BATCH_INTERVAL = 1 * time.Millisecond // test
// const BATCH_INTERVAL = 250 * time.Microsecond // test
// const BATCH_INTERVAL = 10 * time.Millisecond // test

const FAST_PATH_TIMEOUT = 5 * 1e6 // 5ms
// const FAST_PATH_TIMEOUT = 38 * time.Millisecond // 5 * 1e6 // 5ms // unused
const COMMIT_GRACE_PERIOD = 10 * time.Millisecond // 10 ms
// const COMMIT_GRACE_PERIOD = 136 * time.Millisecond // 10 ms
// const COMMIT_GRACE_PERIOD = 300 * time.Millisecond // 10 ms
...
// const PINGPONG = false // no pingpong
const PINGPONG = true // pingpong
const PINGPONG_TIMEOUT_ENABLED = true

const PINGPONG_TIMEOUT = 250 * time.Microsecond

// const PINGPONG_TIMEOUT = 1000 * time.Microsecond

// const PINGPONG_TIMEOUT = 180 * time.Millisecond

// const PINGPONG_TIMEOUT = 56 * time.Millisecond

// const PINGPONG_TIMEOUT = 100 * time.Millisecond // seems to work better

// const PINGPONG_TIMEOUT = 500 * time.Millisecond
...
type Stats struct {
	...
	ppTos         uint64 // number of times a propose was sent by ping-pong timeout
	ppCmds        uint64 // number of cmds sent via ping-pong timeout
	recoveries    uint64
	cmdsRecovered uint64
}
...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool, rreply bool, slowdownDuration time.Duration) *Replica {
	r := &Replica{
...
func (r *Replica) pingpongTimeoutClock() {
	// log.Printf("In pingpongTimeoutClock\n")
	if r.IsLeader2 {
...
func (r *Replica) run() {
	...
	// set slowdown injection stuff for this and genericsmr flag
	...
	// if INJECT_TRANSIENT_SLOWDOWN && r.IsLeader1 {
	...
	for !r.Shutdown {
		...
		case propose := <-onOffProposeChan:
			...
			// if INJECT_SLOWDOWN && !slowdowntimers.BeginFlag {
			...
			if MAX_BATCH > 100 || PINGPONG {
				dlog.Printf("Setting onOffProposeChan to nil\n")
				onOffProposeChan = nil
			}
			dlog.Printf("LastSent diff %v\n", time.Now().Sub(lastSent))
			lastSent = time.Now()

		case <-fastClockChan:
			var adaptedTO = BATCH_INTERVAL
			if PINGPONG {
				adaptedTO = PINGPONG_TIMEOUT
			}
			...
			// cch: I believe this might as well be outside of the loop
			// the channel only makes sure PINGPONG_TIMEOUT amount of time has passed since last I checked...
			if time.Since(lastSent) >= adaptedTO {
				// if there was nothing waiting we act like we sent something...
				if len(r.ProposeChan) == 0 {
					lastSent = time.Now()
				} else {
					r.stat.ppTos++
					r.stat.ppCmds += uint64(len(r.ProposeChan))
					onOffProposeChan = r.ProposeChan
				}
			}
		...
		case prepareS := <-r.prepareChan:
			...
			if prepare.Replica < int32(len(r.PeerAddrList)) && prepare.Replica >= 0 {
		...
		case preAcceptS := <-r.preAcceptChan:
			...
			if preAccept.Replica < int32(len(r.PeerAddrList)) && preAccept.Replica >= 0 {
		...
		case acceptS := <-r.acceptChan:
			...
			if accept.Replica < int32(len(r.PeerAddrList)) && accept.Replica >= 0 {
		...
		case commitS := <-r.commitChan:
			...
			if commit.Replica < int32(len(r.PeerAddrList)) && commit.Replica >= 0 {
		...
		case prepareReplyS := <-r.prepareReplyChan:
			...
			if prepareReply.Replica < int32(len(r.PeerAddrList)) && prepareReply.Replica >= 0 {
		...
		case preAcceptReplyS := <-r.preAcceptReplyChan:
			...
			if preAcceptReply.Replica < int32(len(r.PeerAddrList)) && preAcceptReply.Replica >= 0 {
		...
		case acceptReplyS := <-r.acceptReplyChan:
			...
			if acceptReply.Replica < int32(len(r.PeerAddrList)) && acceptReply.Replica >= 0 {
		...
		case <-r.OnClientConnect:
			dlog.Printf("weird %d; conflicted %d; slow %d; happy %d\n", weird, conflicted, slow, happy)
		...
		for q := 0; q < NUM_LEADERS; q++ {
			...
			for inst = r.ExecedUpTo[q] + 1; inst < r.crtInstance[q]; inst++ {
				...
				if currInst.Status == copilotproto.COMMITTED {
					if RECORD_EXEC_STATS && execLat[q][inst] == nil {
...
func (r *Replica) executeBatch(leaderId int, instNum int32) bool {
	...
	instExec := false
	// if instExected != 0 {
	// 	log.Printf("Start executing cmds in instance %d.%d with %d cmds, average cmds per instance is %d.\n", leaderId, instNum, len(inst.Cmds), totalCmdExected/instExected)
	// }
	for j := 0; j < len(inst.Cmds); j++ {

		reqSt := time.Now()
		// cch: Copilot deduplication
		clientId := inst.Cmds[j].ClientId
		opId := inst.Cmds[j].OpId
		k := getKeyForExecMap(clientId, opId)
		//_, exists := r.execMap[k]
		...
		if RECORD_EXEC_STATS {
			...
			execLat[leaderId][instNum].execed = time.Now()
			log.Printf("Replica %v: (%v.%v): commit %v; execStart %v; execWait %v, execed %v\n", r.Id, leaderId, instNum,
				inst.committedTime.Sub(inst.startTime),
				execLat[leaderId][instNum].start.Sub(inst.startTime),
				execLat[leaderId][instNum].wait.Sub(inst.startTime),
				execLat[leaderId][instNum].execed.Sub(inst.startTime),
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	if PRINT_STATS && r.stat.batches%1 == 0 {
		dlog.Printf("Replica %d: batches: %v, cmds: %v, avg. cmds/batch: %.2f, "+
			"fast: %v, slow: %v,"+
			"rpcsSent: %v, rpcsRcvd: %v, total rpcs: %v, avg. rpcs/batch: %.2f, avg. rpcs/cmd: %.2f"+
			" ppTos: %v, ppCmds: %v, recoveries: %v cmdsRecovered: %v\n"+
			"\n",
			...
			r.stat.ppTos, r.stat.ppCmds, r.stat.recoveries, r.stat.cmdsRecovered)
	}
...
func (r *Replica) startRecoveryForInstance(replica int32, instance int32) {
	...
	if r.views[replica].role != viewchangeproto.ACTIVE {
		...
		rep := replica
		ins := instance
		go func(rep, ins int32) {
			time.Sleep(1 * time.Millisecond)
			r.instancesToRecover <- &instanceId{rep, ins}
		}(rep, ins)
		return
	}
	...
	r.stat.recoveries++
	r.stat.cmdsRecovered += uint64(len(inst.Cmds))
...
func (r *Replica) handleTryPreAcceptReply(tpar *copilotproto.TryPreAcceptReply) {
	...
	if tpar.OK == TRUE {
		...
		if inst.lb.preAcceptOKs >= r.N/2 {
			...
			inst.lb.acceptOKs = 0
			inst.acceptBallot = inst.ballot
			r.bcastAccept(tpar.Replica, tpar.Instance, r.views[tpar.Replica].view.ViewId, inst.ballot, inst.Cmds, inst.Deps, r.views[1-tpar.Replica].view.ViewId)
	...
	if acceptViewReply.OK == TRUE {
		...
		if currViewState.vmb.acceptViewOKs+1 > r.N>>1 {
			...
			if currViewState.view.ReplicaId == r.Id {
				...
				pilot := acceptViewReply.PilotId
				go func(pilot int32) {
					time.Sleep(500 * time.Microsecond)
					for i := r.CommittedUpTo[acceptViewReply.PilotId] + 1; i < r.crtInstance[acceptViewReply.PilotId]; i++ {
						r.instancesToRecover <- &instanceId{pilot, i}
					}
					return
				}(pilot)
			} else {
