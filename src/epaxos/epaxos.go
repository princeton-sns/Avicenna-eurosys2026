...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool, slowdownDuration time.Duration) *Replica {
	r := &Replica{
...
func (r *Replica) printStats() {
	log.Printf("Fast path %d, slow path %d.\n", happy, slow)
}
...
func (r *Replica) handlePreAcceptReply(pareply *epaxosproto.PreAcceptReply) {
	...
	} else if inst.lb.preAcceptOKs >= r.N/2 && (!inst.lb.allEqual || !allCommitted || !isInitialBallot(inst.ballot)) {
		if !allCommitted {
			weird++
		}
		slow++
