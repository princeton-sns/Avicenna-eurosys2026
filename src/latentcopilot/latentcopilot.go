...
const MAX_CLIENTS = 2048 // increase this if experimenting with >100 clients
...
// const BEACON_FAILURE_THRESHOLD = 10000 // cch testing something
...
// const BATCH_INTERVAL = 250 * time.Microsecond

const BATCH_INTERVAL = 10 * time.Millisecond

// const FAST_PATH_TIMEOUT = 5 * 1e6 // 5ms
const FAST_PATH_TIMEOUT = 100 * time.Millisecond //5 * 1e6 // 5ms
// const COMMIT_GRACE_PERIOD = 10 * time.Millisecond
const COMMIT_GRACE_PERIOD = 136 * time.Millisecond
...
const INJECT_LONGLIVED_SLOWDOWN = false
...
const PINGPONG_TIMEOUT = 160 * time.Millisecond
...
const LATENT_PIILOT_DELAY = 136 * time.Millisecond
const SWITCHING_MODE_WINDOW = 1 * time.Second
const MIN_NEW_PROPOSALS_TO_BECOME_ACTIVE = 3
const MIN_NEW_COMMITS_TO_BECOME_LATENT = 3
	...
	for !r.Shutdown {
		...
		case <-slowClockChan:
			...
			for q := int32(0); q < int32(r.N); q++ {
				...
				if r.beaconMisses[q] >= BEACON_FAILURE_THRESHOLD && (q == 0 || q == 1) {
					...
					* no new pilot has been elected, the inner block will try doing view change again.
					...
					// Approach 2
					if r.Alive[q] {
						log.Printf("Replica %v: detect replica %v dead\n", r.Id, q)
					}
					// Start view change for pilot roles which this replica holds
					// TODO: should have a map from replicaId to pilot indices
					for i := 0; i < NUM_LEADERS; i++ {
