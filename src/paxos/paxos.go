...
// const BATCH_INTERVAL = 0 * time.Microsecond
...
func (r *Replica) run() {
	...
	if INJECT_SLOWDOWN && r.IsSlowdownReplica {
...
func (r *Replica) bcastCommit(instance int32, ballot int32, command []state.Command) {
	...
	pc.Command = command
	args := &pc
	// in the FVC version this was commented out and argsShort is never used
	// not sure why that is the case, but I remember argsShort being suspicious
	pcs.LeaderId = r.Id
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	} else { // cch: I am the leader fast path
		r.instanceSpace[instNo] = &Instance{
			cmds,
			r.defaultBallot,
			PREPARED,
...
func (r *Replica) handleCommitShort(commit *paxosproto.CommitShort) {
	...
	// cch: when does it get the command if it commits short but didn't receive
	// 		the accept?
	if inst == nil {
