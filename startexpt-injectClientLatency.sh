...
proto=$1   # protocol to run: "copilot", "latentcopilot", "epaxos", "mr99rsm", "fvc", "multipaxos" (default)
n=$2       # number of replicas
clients=$3 # number of client threads
cnodes=$4  # number of client machines
length=$5  # length of the experiment (in seconds)
percent_mocked=$6  # percent of commands being mock processed
append=$7  # append name in the results directory
slowduration=$8    # length of long-lived slowdown (in millisecond)
...
for i in $(seq 2 $((n + 1))); do
  ...
  -slowdownduration=0 -exec=$exec -dreply=$reply -durable=$durable -p=20 -thrifty=$thrifty \
...
# for j in $(seq 0 $((processes_per_node - 1))); do
    ...
    cd $prefix; \
    bash startClients-test.sh '-maddr=${masterAddr} -mport=${masterPort} -q=$reqs -check=true \
      ...
      $nodeId $exp_uid $clientId $((clientId+processes_per_node-1)) $append " &
      # quit
      # sleep .05
    pid=$!
    pids="$pids $pid"
...
for i in $(seq 2 2); do
  ssh node-$i -o StrictHostKeyChecking=no "\
	cd $prefix; \
	cd scripts/tcwan; \
	sudo ./add_client-latency.sh ${slowduration}"
done
...
# 1. Copy master logs from node-1
# scp node-1:~/${exp_uid}/master*.txt "$final_dir/" 2>/dev/null
# ssh node-1 "rm -rf ~/${exp_uid}"

# 2. Copy server logs from node-2 to node-8
for i in $(seq 2 $((n + 1))); do
  scp node-$i:${logOutputDir}/server-$i.txt "$final_dir/" 2>/dev/null
  ssh node-$i "sudo rm -rf ~/experiments"
done

# 3. Copy client logs from node-9 to node-22
for i in $(seq 0 $((cnodes - 1))); do
  nodeId=$(($i + offset))
  scp node-$nodeId:${logOutputDir}/client-*.out "$final_dir/" 2>/dev/null
  ssh node-$nodeId "sudo rm -rf ~/experiments"
done

scp ${outputDir}/*.txt "$final_dir/" 2>/dev/null
rm -rf experiments

# # 4. Copy experiment-wide summaries from first client node
# scp node-7:~/${exp_uid}/tput*.txt "$final_dir/" 2>/dev/null
# scp node-7:~/${exp_uid}/percentiles*.txt "$final_dir/" 2>/dev/null
# scp node-7:~/${exp_uid}/*.prof "$final_dir/" 2>/dev/null
# ssh node-7 "rm -rf ~/${exp_uid}"
