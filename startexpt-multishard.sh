...
serverFile1="/proj/cops/zijian/Avicenna_release/serverconfig1.txt"
serverFile2="/proj/cops/zijian/Avicenna_release/serverconfig2.txt"
serverFile3="/proj/cops/zijian/Avicenna_release/serverconfig3.txt"
serverFile4="/proj/cops/zijian/Avicenna_release/serverconfig4.txt"
serverFile5="/proj/cops/zijian/Avicenna_release/serverconfig5.txt"
serverFile6="/proj/cops/zijian/Avicenna_release/serverconfig6.txt"
serverFile7="/proj/cops/zijian/Avicenna_release/serverconfig7.txt"
...
# 1) Masters
for s in $(seq 1 $shards); do
  scp -v node-1:${logOutputDir}/master-shard${s}.txt "$final_dir/" 2>/dev/null 
done

# 2) Servers (match filenames with -shard suffix)
for i in $(seq 2 $((n + 1))); do
  for s in $(seq 1 $shards); do
    scp -v node-$i:${logOutputDir}/server${i}-shard${s}.txt "$final_dir/" 2>/dev/null 
  done
  ssh -o StrictHostKeyChecking=no node-$i "rm -rf '${logOutputDir}' ~/experiments" 2>/dev/null 
done

# 3) Clients (optional *.out)
for i in $(seq 0 $((cnodes - 1))); do
  nodeId=$(($i + offset))
  scp -v node-$nodeId:${logOutputDir}/*.out "$final_dir/" 2>/dev/null
  ssh node-$nodeId "rm -rf ~/experiments"
done

# 4) Copy summaries from shared outputDir
scp -v ${outputDir}/tput.txt "$final_dir/" 2>/dev/null
scp -v ${outputDir}/percentilesnew.txt "$final_dir/" 2>/dev/null
scp -v ${outputDir}/sys_tput.txt "$final_dir/" 2>/dev/null
