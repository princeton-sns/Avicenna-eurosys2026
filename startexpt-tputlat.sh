...
for i in $(seq 2 $((n + 1))); do
  ...
  -slowdownduration=$slowduration -exec=$exec -dreply=$reply -durable=$durable -p=16 -thrifty=$thrifty \
...
# for j in $(seq 0 $((processes_per_node - 1))); do
    ...
    bash startClients-test.sh '-maddr=${masterAddr} -mport=${masterPort} -q=$reqs -check=true \
      ...
      -trim=${trim} -w=$writes -proxy=$proxyReplica -p=16 -tput_interval_in_sec=${tput_interval_in_sec} \
...
# 2. Copy server logs from node-2 to node-6
for i in $(seq 2 $((n + 1))); do
  scp node-$i:${logOutputDir}/server-$i.txt "$final_dir/" 2>/dev/null
  ssh node-$i "rm -rf ~/experiments"
done

# 3. Copy client logs from node-7 to node-10
for i in $(seq 0 $((cnodes - 1))); do
  nodeId=$(($i + offset))
